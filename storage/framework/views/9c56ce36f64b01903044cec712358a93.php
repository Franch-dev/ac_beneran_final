...
<?php $__env->startSection('title', 'Monitoring - AC Servis Masjid'); ?>
...
<div class="page-container">
    ...
    <div class="search-bar">
        <form action="<?php echo e(route('monitoring')); ?>" method="GET" class="search-form">
            ...
            <?php if(request()->anyFilled(['search', 'status'])): ?>
                <a href="<?php echo e(route('monitoring')); ?>" class="btn btn-secondary">Reset</a>
            <?php endif; ?>
        </form>
    </div>
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        ...
                        <div class="text-sm text-muted"><?php echo e($order->created_at->format('d M Y')); ?></div>
                    </td>
                    <td>
                        <div class="fw-bold"><?php echo e($order->masjid->name); ?></div>
                        <div class="text-sm text-muted"><?php echo e($order->masjid->custom_id); ?></div>
                    </td>
                    <td>
                        <div><?php echo e($order->service_date->format('d M Y')); ?></div>
                        <div class="text-sm <?php echo e($order->service_date < now() ? 'text-danger' : 'text-success'); ?>">
                            <?php echo e($order->service_date < now() ? 'Lewat' : 'Mendatang'); ?>

                        </div>
                    </td>
                    <td>
                        <?php $__currentLoopData = $order->serviceDetails; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $detail): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="detail-chip"><?php echo e($detail->pk_type); ?> <?php echo e($detail->brand); ?> × <?php echo e($detail->quantity); ?></div>
                    ...
                    <td>
                        ...
                        ?>
                        <span class="urgency-badge urgency-text-<?php echo e($urgency); ?>">
                    ...
                    <td>
                        <div class="action-btns">
                            <button class="btn btn-sm btn-info" onclick="showOrderDetail(<?php echo e($order->id); ?>)">
                                <i class="fas fa-eye"></i>
                            </button>

                            <?php if(auth()->user()->isManager()): ?>
                                <?php if($order->status === 'pending'): ?>
                                <button class="btn btn-sm btn-success" onclick="approveOrder(<?php echo e($order->id); ?>)">
                                    <i class="fas fa-check"></i> Approve
                                </button>
                                <?php elseif($order->status === 'approved'): ?>
                                <button class="btn btn-sm btn-warning" onclick="cancelApprove(<?php echo e($order->id); ?>)">
                                    <i class="fas fa-undo"></i> Batal
                                </button>
                                <button class="btn btn-sm btn-danger" onclick="deleteOrder(<?php echo e($order->id); ?>)">
                                    <i class="fas fa-trash"></i>
                                </button>
                                <?php endif; ?>
                            <?php endif; ?>

                            <?php if($order->status === 'approved' && (auth()->user()->isFrontdesk() || auth()->user()->isManager())): ?>
                                <a href="<?php echo e(route('spk.print', $order->id)); ?>" target="_blank" class="btn btn-sm btn-secondary">
                                    <i class="fas fa-print"></i> SPK
                                </a>
                                <?php if($order->invoice): ?>
                                <a href="<?php echo e(route('invoice.print', $order->id)); ?>" target="_blank" class="btn btn-sm btn-primary">
    ...
    <div class="empty-state">
        ...
        <p><?php echo e(request()->anyFilled(['search', 'status']) ? 'Tidak ada hasil untuk filter tersebut.' : 'Belum ada service order yang dibuat.'); ?></p>
    </div>
    <?php endif; ?>
    ...
    <div class="urgency-grid">
        ...
        <div class="urgency-card urgency-card-<?php echo e($masjid->urgency_status); ?>">
            <div class="urgency-card-header">
                <span class="urgency-card-id"><?php echo e($masjid->custom_id); ?></span>
                <span class="urgency-dot urgency-<?php echo e($masjid->urgency_status); ?>"></span>
            </div>
            <div class="urgency-card-name"><?php echo e(Str::limit($masjid->name, 30)); ?></div>
            <div class="urgency-card-info">
                <span><?php echo e($masjid->acUnits->sum('quantity')); ?> unit</span>
                <span><?php echo e($masjid->max_days_since_service ? $masjid->max_days_since_service . ' hari' : '–'); ?></span>
            </div>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
    ...
    <div class="popup-body popup-two-col">
        ...
        <div class="popup-col-left">
            ...
            <div class="masjid-select-list" id="masjidSelectList">
                <?php $__currentLoopData = $masjids; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="masjid-select-item"
                     data-id="<?php echo e($m->id); ?>"
                     data-name="<?php echo e($m->name); ?>"
                     data-address="<?php echo e($m->address); ?>"
                     data-dkm="<?php echo e($m->dkm_name); ?>"
                     data-marbot="<?php echo e($m->marbot_name); ?>"
                     data-phone="<?php echo e(json_encode($m->phone_numbers)); ?>"
                     data-ac="<?php echo e(json_encode($m->acUnits)); ?>"
                     data-type="<?php echo e($m->type); ?>"
                     onclick="selectMasjidForSO(this)">
                    <div class="msi-id"><?php echo e($m->custom_id); ?></div>
                    <div class="msi-name"><?php echo e($m->name); ?></div>
                    <div class="msi-units"><?php echo e($m->acUnits->sum('quantity')); ?> unit AC</div>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </div>
        ...
        <div class="popup-col-right">
            <div id="soFormContent" style="display:none">
                ...
                <div class="form-group">
                    ...
                    <input type="date" id="soServiceDate" class="form-input" min="<?php echo e(date('Y-m-d')); ?>">
                </div>
...
<div class="popup popup-lg" id="orderDetailPopup">
    ...
    <div class="popup-body" id="orderDetailBody">
        <!-- Dynamic -->
        <?php if(isset($order) && !empty($order->notes)): ?>
...
<script>
const ROUTES_MON = {
    soStore: '<?php echo e(route("service-order.store")); ?>',
...
const isManager = <?php echo e(auth()->user()->isManager() ? 'true' : 'false'); ?>;
const isFrontdesk2 = <?php echo e(auth()->user()->isFrontdesk() ? 'true' : 'false'); ?>;
</script>
<script src="<?php echo e(asset('js/monitoring.js')); ?>"></script>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Hype G12\ac_beneran_final\resources\views\monitoring.blade.php ENDPATH**/ ?>