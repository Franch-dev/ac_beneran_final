...
<div class="page-container page-operations page-operations--monitoring">
    ...
    <section class="search-bar ops-control-bar" data-aos="fade-up" data-aos-delay="120">
        ...
        <form action="{{ route('modules.ac-masjid-musholla.monitoring') }}" method="GET" class="search-form search-form--hero">
            ...
            @if(request()->anyFilled(['search', 'status']))
                <a href="{{ route('modules.ac-masjid-musholla.monitoring') }}" class="btn btn-secondary">Reset</a>
            @endif
        </form>
    </section>
...
onclick="showOrderDetail({{ $order->id }}, '{{ $order->order_number }}', '{{ $order->masjid->name }}', '{{ $order->service_date->format('d M Y') }}')">
...
data-service-date="{{ $order->service_date->format('d M Y') }}">
                                <i class="fas fa-eye"></i> Detail
                            </button>
