...
<section class="section">
    <div class="container" style="max-width: 920px;">
        <div class="section-header" style="text-align:left; margin-bottom: 28px;">
            ...
            <p>Modul informasi layanan, harga infaq servis, dan pantauan unit AC untuk anggota dalam jejaring Forkis. Dashboard dan monitoring membutuhkan login.</p>
        </div>

        <div class="info-banner" style="margin-bottom: 24px;">
            <i class="fas fa-users"></i>
            Ringkasan saat ini: <strong>{{ $totalAnggota }}</strong> anggota tercatat,
            <strong>{{ $totalUnit }}</strong> unit AC terkelola (sumber data domain anggota).
        </div>

        <div class="hero-actions" style="flex-wrap: wrap; gap: 12px;">
            <a href="{{ route('login', ['redirect' => '/ac-anggota/dashboard']) }}" class="btn btn-primary btn-lg">
                <i class="fas fa-th-large"></i> Dashboard
            </a>
            <a href="{{ route('login', ['redirect' => '/ac-anggota/monitoring']) }}" class="btn btn-outline btn-lg">
