...
class WorkflowController extends Controller
{
    ...
    public function assign(Request $request, ServiceOrder $serviceOrder): JsonResponse
    {
        ...
        if (!in_array($serviceOrder->status, ['spk_invoice_approved', 'payment_verified'], true)) {
            return ApiResponse::error('Order harus diverifikasi pembayarannya sebelum bisa ditugaskan.', 422);
        }
    ...
    public function close(Request $request, ServiceOrder $serviceOrder): JsonResponse
    {
        ...
        if ($serviceOrder->status !== 'payment_verified') {
            return ApiResponse::error('Order tidak dalam status yang bisa ditutup.', 422);
        }
    ...
    public function approveSpkInvoice(Request $request, ServiceOrder $serviceOrder): JsonResponse
    {
        ...
        return DB::connection('ac_service')->transaction(function () use ($validated, $serviceOrder) {
            $serviceOrder->update(['status' => 'spk_invoice_approved']);
            ...
            return ApiResponse::success([], 'SPK & Invoice disetujui. Order siap ditugaskan ke teknisi.');
        });
    }
