                ...
                <tr>
                    ...
                    <td class="table-cell-actions">
                        @php
                            ...
                            $canAssignTech = $isManagerOrAdmin && $isSpkApproved;
                        ...
                        <div class="action-btns action-btns--dense">
                            ...
                            @if($isPaymentVerified && $isManagerOrAdmin)
                            <button class="btn btn-sm btn-primary" type="button" onclick="finalizeOrder({{ $order->id }}, @json($order->order_number), @json($masjidName), @json($order->service_date->format('d M Y')), @json($orderStatus))">
    ...
    <div class="monitoring-mobile-list">
        ...
        <article class="monitoring-mobile-card" data-aos="fade-up" data-aos-delay="{{ ($loop->index % 5) * 50 }}">
            ...
            <div class="action-btns">
                ...
                @if((auth()->user()->isManager() || auth()->user()->isAdmin()) && $order->status === 'spk_invoice_approved')
                <button class="btn btn-sm btn-outline btn-accent" type="button"
                ...
                @if($order->status === 'payment_verified' && $mobileIsManagerOrAdmin)
                <button class="btn btn-sm btn-primary" type="button" onclick="finalizeOrder({{ $order->id }}, @json($order->order_number), @json($masjidName), @json($order->service_date->format('d M Y')), @json($order->status))">
                    <i class="fas fa-check-double"></i> Selesaikan Order
                </button>
                @endif
...
(function () {
    ...
    window.approveOrder = async function (orderId, orderNumber, masjidName, serviceDate) {
                ...
                onConfirm: async () => {
                    try {
                        const url = (typeof ROUTES_MON !== 'undefined' && ROUTES_MON.soApprove)
                            ? ROUTES_MON.soApprove(orderId)
                            : `/service-order/${orderId}/approve`;

                        await fallbackFetch(url, 'POST');

                        if (typeof window.showToast === 'function') {
                            window.showToast('Order berhasil disetujui.', 'success');
                        }

                        if (typeof window.refreshMonitoringSurface === 'function') {
                            window.refreshMonitoringSurface();
                            return;
                        }
                        ...
                        if (typeof window.showToast === 'function') {
                            window.showToast('Gagal menyetujui order: ' + error.message, 'error');
                            return;
                        }
    ...
    window.approveSpkInvoice = async function (orderId, orderNumber, masjidName, serviceDate) {
        if (typeof window.openConfirmModal === 'function') {
            window.openConfirmModal({
                type: 'success',
                ...
                onConfirm: async () => {
                    try {
                        const url = (typeof ROUTES_MON !== 'undefined' && ROUTES_MON.workflowApproveSpkInvoice)
                            ? ROUTES_MON.workflowApproveSpkInvoice(orderId)
                            : `/workflow/${orderId}/approve-spk-invoice`;

                        await fallbackFetch(url, 'POST');

                        if (typeof window.showToast === 'function') {
                            window.showToast('SPK & Invoice berhasil disetujui.', 'success');
                        }
                        ...
                        if (typeof window.showToast === 'function') {
                            window.showToast('Gagal menyetujui SPK & Invoice: ' + error.message, 'error');
                            return;
                        }
